...
import { syncCapture, type YukAnalysis } from "@/lib/api";
...
export default function CaptureScreen() {
    ...
    try {
      ...
      if (kind === "litter") {
        ...
        if (locationPermission.status !== "granted") {
          throw new Error("YUK needs location permission to map litter.");
        }
      ...
      try {
        ...
        setMessage(
          result.persisted.siteId
            ? "Synced. This bite is now part of a persistent waste site."
  ...
  if (!permission) {
    return <View style={{ flex: 1, backgroundColor: "#f4f0e7" }} />;
  }
  ...
  if (!permission.granted) {
    return (
      <ScrollView
        ...
        contentContainerStyle={{ padding: 22, gap: 18, alignItems: "stretch" }}
      >
        ...
        <Text selectable style={{ fontSize: 18, lineHeight: 24 }}>
          Camera access is required because the photograph is the evidence. You can still choose whether litter becomes public.
        </Text>
        <Pressable
          onPress={requestPermission}
          style={{ padding: 16, borderWidth: 2, borderRadius: 18, borderCurve: "continuous", backgroundColor: acid }}
        >
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ink }}>
      ...
      >
        <Text selectable style={{ fontWeight: "900", fontSize: 18 }}>
          {kind === "litter" ? "Show YUK the waste where you found it." : "Show YUK what you are throwing away."}
        </Text>
